    ...
    if (locked) {
      return (
        <Link
          ...
          className={`group/i flex items-center gap-3 rounded-2xl py-2.5 text-sm font-semibold text-[hsl(var(--muted-foreground)/0.5)] transition hover:bg-[hsl(var(--accent))] hover:text-[hsl(var(--muted-foreground))] ${rail ? "justify-center px-2 group-hover:justify-start group-hover:px-3" : "px-3"}`}
        >
          ...
          <span className={rail ? "hidden flex-1 group-hover:block" : "flex-1"}>{label(item)}</span>
          <Lock className={`h-3 w-3 opacity-60 ${rail ? "hidden group-hover:block" : ""}`} />
        </Link>
      );
    }
    ...
    return (
      <Link
        ...
        className={`group/i flex items-center gap-3 rounded-2xl py-2.5 text-sm font-semibold transition ${
          rail ? "justify-center px-2 group-hover:justify-start group-hover:px-3" : "px-3"
        } ${
          isActive
      ...
      >
        ...
        <span className={rail ? "hidden flex-1 group-hover:block" : "flex-1"}>{label(item)}</span>
        <ChevronRight className={`h-3 w-3 transition ${rail ? "hidden group-hover:block" : ""} ${isActive ? "opacity-70" : "opacity-0 group-hover/i:opacity-40"}`} />
      </Link>
    );
  }

  return (
    <nav className={`flex-1 overflow-y-auto ${rail ? "p-2 group-hover:p-3" : "p-3"}`}>
      {NAV_GROUPS.map((group, gi) => (
        <div key={gi} className={group.label ? "mt-4 first:mt-0" : ""}>
          {group.label && (
            <p className={`mb-1 px-3 text-[10px] font-bold uppercase tracking-widest text-[hsl(var(--muted-foreground)/0.6)] ${rail ? "hidden group-hover:block" : ""}`}>
