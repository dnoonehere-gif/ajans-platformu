    ...
    <div className="flex min-h-screen items-center justify-center bg-[#eceae5] px-6">
      <div className="w-full max-w-md rounded-3xl border border-white/[0.08] bg-[#121218] p-8 text-center shadow-2xl">
        <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-600 to-indigo-600">
          <MailCheck className="h-8 w-8 text-[var(--mk-ink)]" />
        </div>
        <h1 className="text-xl font-bold text-[var(--mk-ink)]">{t.title}</h1>
        <p className="mt-3 text-sm leading-relaxed text-[var(--mk-ink-soft)]">{t.desc}</p>

        {email && (
          <div className="mt-5 rounded-xl border border-neutral-900/[0.07] bg-neutral-900/[0.03] px-4 py-3">
            <p className="text-xs text-[var(--mk-ink-soft)]">{t.sentTo}</p>
            <p className="mt-1 break-all text-sm font-semibold text-violet-300">{email}</p>
          </div>
        )}

        <p className="mt-4 text-xs text-[var(--mk-ink-soft)]">{t.notReceived}</p>
        ...
        <button
          ...
          className="mt-3 w-full rounded-xl border border-[var(--mk-line)] py-3 text-sm font-medium text-[var(--mk-ink-soft)] transition hover:bg-[var(--mk-chip)]"
        >
          {t.refresh}
        </button>
        ...
        <button
          ...
          className="mx-auto mt-5 flex items-center gap-1.5 text-xs text-[var(--mk-ink-soft)] transition hover:text-[var(--mk-ink-soft)]"
        >
