    ...
    <>
      ...
      <section className="relative overflow-hidden px-6 py-20">
        ...
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h1 className="mt-5 text-4xl font-black leading-tight text-[var(--mk-ink)] sm:text-5xl">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-[var(--mk-ink-soft)]">
            {s.heroDesc}
          </p>
        </div>
      </section>
        ...
        <div className="mx-auto grid max-w-4xl grid-cols-2 gap-4 sm:grid-cols-4">
          {s.stats.map((st) => (
            <div key={st.label} className="rounded-2xl border border-neutral-900/[0.08] bg-neutral-900/[0.03] p-6 text-center">
              <p className="text-2xl font-black text-[var(--mk-ink)] sm:text-3xl">{st.value}</p>
              <p className="mt-1 text-xs font-medium text-[var(--mk-ink-soft)]">{st.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-6 py-16">
        <div className="mx-auto grid max-w-5xl gap-8 md:grid-cols-2">
          ...
          <div className="rounded-3xl border border-neutral-900/[0.08] bg-neutral-900/[0.03] p-8">
            ...
            <h2 className="mt-4 text-2xl font-bold text-[var(--mk-ink)]">{s.visionTitle}</h2>
            <p className="mt-3 leading-relaxed text-[var(--mk-ink-soft)]">{s.visionDesc}</p>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 py-8">
        <div className="mx-auto max-w-5xl">
          <div className="mb-10 text-center">
            <h2 className="text-3xl font-black text-[var(--mk-ink)]">{s.valuesTitle}</h2>
            <p className="mt-3 text-[var(--mk-ink-soft)]">{s.valuesDesc}</p>
          </div>
            ...
            {s.values.map((v, i) => {
              ...
              return (
                <div key={v.title} className="flex gap-4 rounded-2xl border border-neutral-900/[0.08] bg-neutral-900/[0.03] p-6 transition hover:-translate-y-0.5 hover:border-violet-400/25 hover:bg-neutral-900/[0.05]">
                  ...
                  <div>
                    <h3 className="font-bold text-[var(--mk-ink)]">{v.title}</h3>
                    <p className="mt-1 text-sm leading-relaxed text-[var(--mk-ink-soft)]">{v.desc}</p>
                  </div>
                </div>
              );
            })}
