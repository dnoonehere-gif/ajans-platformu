    ...
    <>
      ...
      <section className="px-6 pb-8">
        <div className="mx-auto grid max-w-4xl grid-cols-2 gap-4 sm:grid-cols-4">
          {s.stats.map((st) => (
            <div key={st.label} className="rounded-2xl border border-neutral-900/[0.08] bg-neutral-900/[0.03] p-6 text-center">
              <p className="text-2xl font-black text-neutral-900 sm:text-3xl">{st.value}</p>
              <p className="mt-1 text-xs font-medium text-neutral-500">{st.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-6 py-16">
        <div className="mx-auto grid max-w-5xl gap-8 md:grid-cols-2">
          <div className="rounded-3xl bg-gradient-to-br from-violet-600 to-indigo-700 p-8 text-neutral-900 shadow-[0_0_50px_-12px_rgba(139,92,246,0.5)]">
          ...
          <div className="rounded-3xl border border-neutral-900/[0.08] bg-neutral-900/[0.03] p-8">
            <Rocket className="h-8 w-8 text-violet-400" />
            <h2 className="mt-4 text-2xl font-bold text-neutral-900">{s.visionTitle}</h2>
            <p className="mt-3 leading-relaxed text-neutral-500">{s.visionDesc}</p>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 py-8">
        <div className="mx-auto max-w-5xl">
          <div className="mb-10 text-center">
            <h2 className="text-3xl font-black text-neutral-900">{s.valuesTitle}</h2>
            <p className="mt-3 text-neutral-500">{s.valuesDesc}</p>
          </div>
          <div className="grid gap-5 sm:grid-cols-2">
            {s.values.map((v, i) => {
              const Icon = VALUE_ICONS[i] ?? Target;
              return (
                <div key={v.title} className="flex gap-4 rounded-2xl border border-neutral-900/[0.08] bg-neutral-900/[0.03] p-6 transition hover:-translate-y-0.5 hover:border-violet-400/25 hover:bg-neutral-900/[0.05]">
      ...
      <section className="px-6 py-20">
        <div className="mx-auto max-w-4xl rounded-3xl border border-violet-400/20 bg-gradient-to-br from-violet-50 to-white p-10 text-center sm:p-14">
          ...
          <h2 className="mt-4 text-3xl font-black text-neutral-900">{s.ctaTitle}</h2>
          <p className="mx-auto mt-3 max-w-md text-neutral-500">{s.ctaDesc}</p>
          <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Link href="/kayit" className="group flex items-center gap-2 rounded-2xl bg-gradient-to-r from-violet-600 to-indigo-600 px-8 py-4 font-bold text-neutral-900 shadow-[0_0_40px_-8px_rgba(139,92,246,0.6)] transition hover:-translate-y-0.5">
              {s.ctaPrimary} <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" />
            </Link>
            <Link href="/iletisim" className="rounded-2xl border border-white/15 px-8 py-4 font-semibold text-neutral-900 transition hover:bg-neutral-900/[0.05]">
              {s.ctaSecondary}
            </Link>
          </div>
        </div>
